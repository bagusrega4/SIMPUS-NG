...
export default function Navigation() {
  ...
  const handleDropdownToggle = (label: string, event?: React.MouseEvent | React.TouchEvent) => {
    event?.preventDefault();
    event?.stopPropagation();
    setActiveDropdown(activeDropdown === label ? null : label);
  };

  const handleNavigation = (href: string, event?: React.MouseEvent | React.TouchEvent) => {
    event?.preventDefault();
    event?.stopPropagation();
    navigate(href);
    setActiveDropdown(null);
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 relative">
          ...
          <Link to="/" className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-lg flex items-center justify-center overflow-hidden">
              <img
                src="https://cdn.builder.io/api/v1/image/assets%2F342866cebe1f4dc6a66579741e43d994%2F75f62761293242469d0bfb786e5146bf?format=webp&width=40"
                alt="SIMPus Logo"
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-lg hover:bg-gray-100 z-50 relative pointer-events-auto"
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && (
          <div className="lg:hidden py-4 border-t border-gray-200 bg-white/95 backdrop-blur-md relative z-40">
            <nav className="space-y-2 max-h-96 overflow-y-auto pointer-events-auto">
              {navItems.map((item) => (
                <div key={item.label}>
                  {item.dropdown ? (
                    <div>
                      <button
                        ...
                        onTouchEnd={(e) => handleDropdownToggle(item.label, e)}
                        ...
                        <div className="pl-4 mt-2 space-y-2">
                          {item.dropdown.map((dropdownItem) => (
                            <button
                              ...
                              onTouchEnd={(e) => handleNavigation(dropdownItem.href, e)}
